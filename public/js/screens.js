...
function displayLeaderboardScreen()     // this function is called every frame while on the "Leaderboard" screen. It's called in the Screen object on screens.
{
    ...
    push()
        ...
        if (localStorage.classCode != "123")
        {
            // levelLeaderboard = currentLeaderboard.filter(entry => entry.course == localStorage.classCode)
            // levelLeaderboard = currentLeaderboard
        }
        for (let i = 0; i < levelLeaderboard.length; i++) 
        {
            ...
            let studentName = stat.username;
            // let className = stat.classCode;
            let className = "PHYS 102";

            let attempt = stat.score;
            let time = stat.timeToComplete;
